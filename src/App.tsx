...
export default function App() {
  ...
  const stats = useMemo(() => {
    ...
    const recentActivityCount = activities.filter(a => a.timestamp && a.timestamp > last7Days).length;
    ...
    return {
      ...
      activityTrend: recentActivityCount > 0 ? `+${recentActivityCount} events` : 'Stable'
    };
  }, [tracks, clients, activities]);
  ...
  const chartData = useMemo(() => {
    ...
    for (let i = 6; i >= 0; i--) {
      ...
      const dayActivities = activities.filter(a => a.timestamp && a.timestamp.startsWith(dateStr));
      const plays = dayActivities.filter(a => a.type === 'play' || (a.action && a.action.includes('played'))).length;
      const eng = dayActivities.filter(a => ['social', 'upload'].includes(a.type || '')).length;
  ...
  const renderDashboard = () => (
    <div className="p-8 space-y-8">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
        {[
          { label: 'Total Tracks', value: stats.totalTracks, icon: Music, trend: 'Catalog', color: 'text-orange-500' },
          { label: 'Active Clients', value: stats.activeClients, icon: Users, trend: 'Network', color: 'text-blue-500' },
          { label: 'Total Plays', value: stats.totalPlays.toLocaleString(), icon: Play, trend: 'Vibes', color: 'text-emerald-500' },
          { label: 'Engagement', value: stats.engagementRate, icon: TrendingUp, trend: stats.activityTrend, color: 'text-purple-500' },
        ].map((stat, i) => (
          <motion.div 
            key={stat.label}
